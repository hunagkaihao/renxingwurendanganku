...
public class DispatchCoreJob : IHostedService, IDisposable
{
    ...
    private Dictionary<int, DispatchTask> TaskDic
    {
        ...
        set
        {
            lock (mTaskDicLocker)
            {
                ...
                _ecsRedisClient.SetStringValue(WcsConsts.DispatchTasksDoing, json);
            }
        }
    }
    ...
    private async Task<bool> OrderToTaskAndSaveAsync(DispatchOrder order)
    {
        ...
        catch (WcsDomainException) //该类型的错误是已经记录在日志中的
        {
            return false;
        }
        catch (Exception ex)
    ...
    public async Task<string> GetServerState()
    {
        ...
        if (state == "Running")
        {
            if (true == _notifierManager.IsNotifierValChanged(WcsConsts.PauseDispatcherSvrNotifierName))
            {
        ...
        {
            if (true == _notifierManager.IsNotifierValChanged(WcsConsts.RunDispatcherSvrNotifierName))
            {
                await UpdateDispatchSvrStateAsync("Running").ConfigureAwait(false);
                state = "Running";
            }
    ...
    {
        if (true != _notifierManager.IsNotifierValWithParaChanged(WcsConsts.DispatchOrderForceDoneNotifierName, out string orderCode))
            return;

        if (ServerState == "Running")
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderForceDoneRespNotifierName, "服务处于运行状态，不能执行强制完成操作");
        ...
        if (order == null)
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderForceDoneRespNotifierName, $"订单码为{orderCode}的订单不存在");
        ...
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderForceDoneRespNotifierName, $"订单码为{orderCode}的订单已结束，无需强制完成");
        ...
        if (result.IsOK != true)
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderForceDoneRespNotifierName, $"强制完成失败");
            _logger.Error($"收到强制完成调度订单请求，订单码为{orderCode}，但强制完成失败 {result.Message}");
            return;
        }

        _notifierManager.NotifyDispatchSvr(WcsConsts.StopCheckOrderNotifierName);//通知盘点Job停止
    ...
    {
        if (true != _notifierManager.IsNotifierValWithParaChanged(WcsConsts.DispatchOrderCancelNotifierName, out string orderCode))
            return;

        if (ServerState == "Running")
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderCancelRespNotifierName, $"服务处于运行状态，不能执行取消操作");
        ...
        if (order == null)
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderCancelRespNotifierName, $"订单码为{orderCode}的订单不存在");
        ...
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderCancelRespNotifierName, $"订单码为{orderCode}的订单已经执行或已经完成，无法取消");
        ...
        if (result.IsOK != true)
        {
            _notifierManager.NotifyDispatchSvrWithPara(WcsConsts.DispatchOrderCancelRespNotifierName, $"取消失败");
    ...
    public void UpdateDispatchSvrState(string state)
    {
        try
        {
            _ecsRedisClient.SetStringValue(WcsConsts.DispatchSvrStateChannel, state);
    ...
    public async Task UpdateDispatchSvrStateAsync(string state)
    {
        try
        {
            await _ecsRedisClient.SetStringValueAsync(WcsConsts.DispatchSvrStateChannel, state).ConfigureAwait(false);
            NotifyClientsToUpdateSvrStatusAsync(state);
        }
        catch (Exception ex)
        {
    ...
    public string GetDispatchSvrState()
    {
        try
        {
            return _ecsRedisClient.GetStringValue(WcsConsts.DispatchSvrStateChannel);
        }
        catch (Exception ex)
        {
            _logger.Error(ex.Message);
    ...
    public async Task<string> GetDispatchSvrStateAsync()
    {
        try
        {
            return await _ecsRedisClient.GetStringValueAsync(WcsConsts.DispatchSvrStateChannel);
        }
        catch (Exception ex)
        {
            _logger.Error(ex.Message);
