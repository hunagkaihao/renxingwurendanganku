...
public class StationManager : IDomainService
{
        ...
        IOptions<ConfigOptions> options)
    {
        ...
        {   
            //从Redis删除所有站点通知器数据
            string[] notifierKeys = _redisClient.GetHashFields(EcsConsts.StationNotifierChannelName);
            string[] notifierTempKeys = _redisClient.GetHashFields(EcsConsts.StationNotifierTmpChannelName);
            _redisClient.RemoveHashFields(EcsConsts.StationNotifierChannelName, notifierKeys);
            _redisClient.RemoveHashFields(EcsConsts.StationNotifierTmpChannelName, notifierTempKeys);
        }
        catch (Exception ex)
        {
    ...
    public void NotifyStation(string notifierName)
    {
        try
        {
            string notifierVal = _redisClient.GetHashValue(EcsConsts.StationNotifierChannelName, notifierName);
            if (notifierVal == null)
                _redisClient.SetHashValue(EcsConsts.StationNotifierChannelName, notifierName, "1");
            else
            {
                ...
                else
                {
                    ...
                    if (val == int.MaxValue)
                        val = 1;
                    _redisClient.SetHashValue(EcsConsts.StationNotifierChannelName, notifierName, val.ToString());
                }
            }
        }
        catch (Exception ex)
    ...
    public bool? IsNotifierValChanged(string notifierName)
    {
        ...
        {
            string notifierVal = _redisClient.GetHashValue(EcsConsts.StationNotifierChannelName, notifierName);
            string notifierTempVal = _redisClient.GetHashValue(EcsConsts.StationNotifierTmpChannelName, notifierName);
            if (notifierVal != notifierTempVal)
            {
                _redisClient.SetHashValue(EcsConsts.StationNotifierTmpChannelName, notifierName, notifierVal);
                return true;
            }
            return false;
        }
