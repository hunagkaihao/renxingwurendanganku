...
{
    public class JudgeJobWorker : IJobWorker, ITransientDependency
    {
        ...
        public async Task Execute()
        {
            ...
            if (state == EnumDispatchJobState.WaitingDone)
            {
                ...
                else
                {
                    ...
                    if (details == null)
                    {
                        string errInfo = $"设备执行完成，查询所属过程节点数据失败";
                        await _orderManager.UpdateExecInfoOfDispatchOrderAsync(MyJob.OrderCode, errInfo, true).ConfigureAwait(false);
                        await _backupManager.UpdateJobExecInfoOfOrderInRedisAsync(MyJob.OrderCode, MyJob.Id, errInfo).ConfigureAwait(false);
                        return;
                    }
                    ...
                    {
                        string errInfo = $"设备执行完成，查询所属过程节点数量为{details.Count}，小于当前节点顺序值{MyJob.ProcessSequence}";
                        await _orderManager.UpdateExecInfoOfDispatchOrderAsync(MyJob.OrderCode, errInfo, true).ConfigureAwait(false);
                        await _backupManager.UpdateJobExecInfoOfOrderInRedisAsync(MyJob.OrderCode, MyJob.Id, errInfo).ConfigureAwait(false);
                        return;
                    }
                    ...
                    int nextStep = 0;
                    if (MyJobCmd.JudgeResult)
                        nextStep = MyJob.NextTrueStep;
                    else
                        nextStep = MyJob.NextFalseStep;

                    if (nextStep == 0) //代表没有下一步，此调度任务完成
                    {
                        bool? res = await _jobManager.UpdateJobsStateAsync(MyJob.TaskId, EnumDispatchJobState.Done).ConfigureAwait(false);
                        if (res == null || res == false)
                        {
                            string errInfo = $"设备执行完成，判断结果为{MyJobCmd.JudgeResult}，且没有下一命令，刷新此任务的所有Job状态为Done失败";
                            await _orderManager.UpdateExecInfoOfDispatchOrderAsync(MyJob.OrderCode, errInfo, true).ConfigureAwait(false);
                            await _backupManager.UpdateJobExecInfoOfOrderInRedisAsync(MyJob.OrderCode, MyJob.Id, errInfo).ConfigureAwait(false);
                            return;
                        }
                        ...
                        {
                            string execInfo = $"Job执行完成，判断结果为{MyJobCmd.JudgeResult}，且没有下一命令，刷新此任务的所有Job状态为Done成功";
                    ...
                    {
                        string errInfo = $"设备执行完成，下一命令为{nextStep}，不在范围{MyJob.ProcessSequence + 1}~{details.Count}内";
                        await _orderManager.UpdateExecInfoOfDispatchOrderAsync(MyJob.OrderCode, errInfo, true).ConfigureAwait(false);
                        await _backupManager.UpdateJobExecInfoOfOrderInRedisAsync(MyJob.OrderCode, MyJob.Id, errInfo).ConfigureAwait(false);
                        return;
                    }
                    ...
                    if (r == null || r == false)
                    {
                        string errInfo = $"设备执行完成，判断结果为{MyJobCmd.JudgeResult}，刷新命令{nextStep}前的所有Job状态为Done失败";
                        await _orderManager.UpdateExecInfoOfDispatchOrderAsync(MyJob.OrderCode, errInfo, true).ConfigureAwait(false);
                        await _backupManager.UpdateJobExecInfoOfOrderInRedisAsync(MyJob.OrderCode, MyJob.Id, errInfo).ConfigureAwait(false);
                        return;
                    }
                    ...
                    {
                        string execInfo = $"Job已完成设备执行，判断结果为{MyJobCmd.JudgeResult}，刷新命令{nextStep}前的所有Job状态为Done成功";
                        await _orderManager.UpdateExecInfoOfDispatchOrderAsync(MyJob.OrderCode, string.Empty, false).ConfigureAwait(false);
                        await _backupManager.UpdateJobExecInfoOfOrderInRedisAsync(MyJob.OrderCode, MyJob.Id, string.Empty).ConfigureAwait(false);
