...
{
    public class XnLastChkTaskJudgeCmd : IJobCmd, ITransientDependency
    {
        ...
        public XnLastChkTaskJudgeCmd(OrderManager orderManager,
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            //获取对应的Job，并读取job的Id
            if (Owner == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"\"最后一个盘点或批量入库任务判断\"命令没有指定所属的JobWorker" };

            if (Owner.MyJob == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"\"最后一个盘点或批量入库任务判断\"命令所属Job为空" };
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            ...
            {
                if (Owner == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个盘点或批量入库任务判断\"命令没有指定所属的JobWorker信息" };

                if (Owner.MyJob == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个盘点或批量入库任务判断\"命令所属Job为空" };


                //该命令用于判断所属的调度任务是否是最后一个盘点或批量入库任务，该调度任务本身必须是盘点或批量入库任务
                ...
                if (order.OrderType != EnumDispatchOrderType.CheckDown)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个盘点或批量入库任务判断\"命令所属调度任务非盘点或批量入库任务" };


                //查询当前正在执行的所有盘点或批量入库任务
                string json = _redisClient.GetStringValue(EcsConsts.DispatchTasksDoing);
                ...
                Dictionary<int, DispatchTask> chkTasksDoing = new Dictionary<int, DispatchTask>();
                foreach (var item in tasksDoing)
                {
                    ...
                    if (order.OrderType == EnumDispatchOrderType.CheckDown)
                        chkTasksDoing.Add(item.Key, item.Value);
                }

                if (chkTasksDoing.Count == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "当前正在执行的调度任务中查询不到盘点或批量入库任务" };

                //判断当前任务是否在其中
                int curTaskIndex = -1;
                for (int i = 0; i < chkTasksDoing.Keys.Count; i++)
                {
                    if (Owner.MyJob.TaskId == chkTasksDoing.Keys.ElementAt(i))
                    {
                        curTaskIndex = i;
                        break;
                    }
                ...
                if (curTaskIndex == -1)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "当前正在执行的盘点或批量入库任务中查询不到当前任务" };


                //判断当前盘点或批量入库任务是否是最后一个盘点或批量入库任务
                if (chkTasksDoing.Count == 1)
                    JudgeResult = true;
                else
                {
                    chkTasksDoing.Remove(Owner.MyJob.TaskId);
                    JudgeResult = true;
                    foreach (var item in chkTasksDoing)
                    {
                ...
                //JudgeResult = curTaskIndex == (chkTasksDoing.Count - 1);
