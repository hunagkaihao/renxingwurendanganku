...
namespace Ecs.Jobs.JobCmds
{
    public class LMToZeroPosCmd : IJobCmd, ITransientDependency
    {
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            ...
            //获取命令值
            var cmd = _nodeManager.GetNodeCmdAsync("13", EcsConsts.NodeType_LMToZeroPos).GetAwaiter().GetResult();
            if (cmd == null)
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            try
            {
                ...
                var nodeCmd = _nodeManager.GetNodeCmdAsync("13", EcsConsts.NodeType_LMToZeroPos).GetAwaiter().GetResult();
                if (nodeCmd == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询龙门回原点命令信息失败" };
