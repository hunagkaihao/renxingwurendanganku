...
{
    public class XnLastOutTaskJudgeCmd : IJobCmd, ITransientDependency
    {
        ...
        public XnLastOutTaskJudgeCmd(OrderManager orderManager,
        ...
        {
            if (Owner == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"\"最后一个出库任务判断\"命令没有指定所属的JobWorker" };

            if (Owner.MyJob == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"\"最后一个出库任务判断\"命令所属Job为空" };
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            ...
            {
                if (Owner == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个出库任务判断\"命令没有指定所属的JobWorker信息" };

                if (Owner.MyJob == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个出库任务判断\"命令所属Job为空" };


                //该命令用于判断所属的调度任务是否是最后一个出库任务，该调度任务本身必须是出库任务
                ...
                if (order.OrderType != EnumDispatchOrderType.StockOut)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个出库任务判断\"命令所属调度任务非出库任务" };
                ...
                Dictionary<int, DispatchTask> outTasksDoing = new Dictionary<int, DispatchTask>();
                foreach (var item in tasksDoing)
                {
                    ...
                    if (order.OrderType == EnumDispatchOrderType.StockOut)
                        outTasksDoing.Add(item.Key, item.Value);
                }

                if (outTasksDoing.Count == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "当前正在执行的调度任务中查询不到出库任务" };


                //判断当前出库任务是否在其中
                int curTaskIndex = -1;
                for (int i = 0; i < outTasksDoing.Keys.Count; i++)
                {
                    if (Owner.MyJob.TaskId == outTasksDoing.Keys.ElementAt(i))
                    {
                        curTaskIndex = i;
                        break;
                    }
                ...
                if (curTaskIndex == -1)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "当前正在执行的出库任务中查询不到当前任务" };


                //判断当前出库任务是否是最后一个出库任务
                if (outTasksDoing.Count == 1)
                    JudgeResult = true;
                else
                {
                    outTasksDoing.Remove(Owner.MyJob.TaskId);
                    JudgeResult = true;
                    foreach (var item in outTasksDoing)
                    {
                ...
                //JudgeResult = curTaskIndex == (outTasksDoing.Count - 1);
