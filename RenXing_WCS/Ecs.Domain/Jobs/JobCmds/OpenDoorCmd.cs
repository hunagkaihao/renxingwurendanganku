...
namespace Ecs.Jobs.JobCmds
{
    public class OpenDoorCmd : IJobCmd, ITransientDependency
    {
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            ...
            //获取命令值
            var cmd = _nodeManager.GetNodeCmdAsync("12", EcsConsts.NodeType_DoorOpen).GetAwaiter().GetResult();
            if (cmd == null)
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            try
            {
                ...
                var nodeCmd = _nodeManager.GetNodeCmdAsync("12", EcsConsts.NodeType_DoorOpen).GetAwaiter().GetResult();
                if (nodeCmd == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询取档口打开命令信息失败" };
