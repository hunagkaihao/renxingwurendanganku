...
{
    public class XnLastInTaskJudgeCmd : IJobCmd, ITransientDependency
    {
        ...
        public XnLastInTaskJudgeCmd(OrderManager orderManager,
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            //获取对应的Job，并读取job的Id
            if (Owner == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"\"最后一个入库任务判断\"命令没有指定所属的JobWorker" };

            if (Owner.MyJob == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"\"最后一个入库任务判断\"命令所属Job为空" };
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            ...
            {
                if (Owner == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个入库任务判断\"命令没有指定所属的JobWorker信息" };
                ...
                if (order.OrderType != EnumDispatchOrderType.StockIn)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"最后一个入库任务判断\"命令所属调度任务非入库任务" };
                ...
                Dictionary<int, DispatchTask> inTasksDoing = new Dictionary<int, DispatchTask>();
                foreach(var item in tasksDoing)
                {
                    ...
                    if (order.OrderType == EnumDispatchOrderType.StockIn)
                        inTasksDoing.Add(item.Key, item.Value);
                }

                if (inTasksDoing.Count == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "当前正在执行的调度任务中查询不到入库任务" };


                //判断当前入库任务是否在其中
                int curTaskIndex = -1;
                for (int i = 0; i < inTasksDoing.Keys.Count; i++)
                {
                    if (Owner.MyJob.TaskId == inTasksDoing.Keys.ElementAt(i))
                    {
                        curTaskIndex = i;
                        break;
                    }
                ...
                if (curTaskIndex == -1)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "当前正在执行的入库任务中查询不到当前任务" };


                //判断当前入库任务是否是最后一个入库任务
                if (inTasksDoing.Count == 1)
                    JudgeResult = true;
                else
                {
                    inTasksDoing.Remove(Owner.MyJob.TaskId);
                    JudgeResult = true;
                    foreach (var item in inTasksDoing)
                    {
                ...
                //JudgeResult = curTaskIndex == (inTasksDoing.Count - 1);
