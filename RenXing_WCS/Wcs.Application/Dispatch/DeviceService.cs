...
public class DeviceService : WcsAppService, IDeviceService
{
    ...
    public ResponseDto OpenDoorAsync(string doorCode)
    {
        try
        {
            ...
            if (!ret)
                return new ResponseDto() { success = false, message = $"向 Plc1.Cmd_{doorCode} 发送开门指令失败" };

            _logger.Info($"成功发送柜门 {doorCode} 开门指令");
            return new ResponseDto() { success = true, message = $"向 Plc1.Cmd_{doorCode} 发送开门指令成功" };
        }
        catch (Exception ex)
        {
            _logger.Error(ex.Message);
    ...
    public async Task<DoorStateDto> GetDoorStateAsync(string doorCode)
    {
        try
        {
            ...
            if (value == null || value.Quality == EnumQuality.Bad)
                return new DoorStateDto() { success = false, message = $"读取变量 Plc1.Status_{doorCode} 失败", doorState = false };
