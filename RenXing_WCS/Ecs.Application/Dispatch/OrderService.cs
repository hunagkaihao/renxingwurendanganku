...
public class OrderService : EcsAppService, IOrderService
{
    ...
    public async Task<ResponseDto> ForceDoneDispatchOrderAsync(ForceDoneDto para)
    {
        try
        {
            _notifierManager.IsNotifierValWithParaChanged(EcsConsts.DispatchOrderForceDoneRespNotifierName, out string msg);//先判断一次，进行复位
            _notifierManager.NotifyDispatchSvrWithPara(EcsConsts.DispatchOrderForceDoneNotifierName, para.orderCode);
            ...
            while (true)
            {
                ...
                if (true == _notifierManager.IsNotifierValWithParaChanged(EcsConsts.DispatchOrderForceDoneRespNotifierName, out msg))
    ...
    public async Task<ResponseDto> CancelDispatchOrderAsync(CancelOrderDto para)
    {
        try
        {
            _notifierManager.IsNotifierValWithParaChanged(EcsConsts.DispatchOrderCancelRespNotifierName, out string msg);//先判断一次，进行复位
            _notifierManager.NotifyDispatchSvrWithPara(EcsConsts.DispatchOrderCancelNotifierName, para.orderCode);
            long firstPoint = DateTime.Now.Ticks;
            while (true)
            {
                await Task.Delay(20).ConfigureAwait(false);
                ...
                long thisPoint = DateTime.Now.Ticks;
                TimeSpan ts = new TimeSpan(thisPoint - firstPoint);
                if (true == _notifierManager.IsNotifierValWithParaChanged(EcsConsts.DispatchOrderCancelRespNotifierName, out msg))
                    return new ResponseDto() { success = msg == string.Empty, message = msg };
