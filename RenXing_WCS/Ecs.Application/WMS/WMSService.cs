...
namespace Ecs.WMS
{
    ...
    /// WMS服务
    /// </summary>
    public class WMSService : EcsAppService, IWMSService
    {
        ...
        private async Task<string> SendRequestWithRedirectHandlingAsync<T>(T requestDto, string endpoint, int retryCount)
        {
            ...
            try
            {
                ...
                _logger.LogDebug("发送请求到WMS，URL：{Url}", $"{_wmsUrl}/{endpoint}");
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    {
                        _logger.LogInformation("遇到302重定向，尝试重新获取token并重试，端点：{Endpoint}", endpoint);
                        ...
                        {
                            _logger.LogInformation("重新获取token成功，开始重试请求，端点：{Endpoint}", endpoint);
                            // 递归重试
                            return await SendRequestWithRedirectHandlingAsync(requestDto, endpoint, retryCount + 1);
                        }
                        else
                        {
                            _logger.LogError("重新获取token失败，无法重试请求，端点：{Endpoint}", endpoint);
                        }
                    }
                }
