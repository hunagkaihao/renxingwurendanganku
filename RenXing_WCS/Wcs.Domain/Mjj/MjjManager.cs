...
public class MjjManager : IDomainService
{
    ...
    public async Task<MjjStatus> GetMjjStatusAync()
    {
        try
        {
            KeyValuePair<string, string>[] pairs = await _redisClient.GetAllHashFieldValuePairsAsync(WcsConsts.MjjStatusChannel).ConfigureAwait(false);
    ...
    public async Task<MjjOpResult> MoveLeftAsync(byte colNo)
    {
        try
        {
            ...
            bool ret = await _redisClient.PublishAsync(WcsConsts.MjjCmdChannel, strMsg).ConfigureAwait(false);
            if(!ret)
