...
public class BackupManager : ISingletonDependency
{
    ...
    {
        string[] unDoneOrderCodes = _ecsRedisClient.GetHashFields(WcsConsts.UnFinishedDispatchOrderCodeChannel);
    ...
    public async Task<List<OrderInRedis>> GetAllOrdersInRedisAsync()
    {
        try
        {
            ...
            KeyValuePair<string, string>[] pairs = await _ecsRedisClient.GetAllHashFieldValuePairsAsync(WcsConsts.DispatchOrderChannel).ConfigureAwait(false);
    ...
    public async Task SetOrderInfoInRedisAsync(OrderInRedis data)
    {
        try
        {
            if (data.orderState != EnumDispatchOrderState.Created.ToString() &&
                ...
                await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel, new string[] { data.orderCode }).ConfigureAwait(false);
            else //订单未完成
                await _ecsRedisClient.SetHashValueAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel, data.orderCode, string.Empty).ConfigureAwait(false);

            string strDto = JsonConvert.SerializeObject(data);
            await _ecsRedisClient.SetHashValueAsync(WcsConsts.DispatchOrderChannel, data.orderCode, strDto).ConfigureAwait(false);
    ...
    public async Task<OrderInRedis> GetOrderWithOrderCodeInRedisAsync(string orderCode)
    {
        try
        {
            string strDto = await _ecsRedisClient.GetHashValueAsync(WcsConsts.DispatchOrderChannel, orderCode).ConfigureAwait(false);
            if (strDto == null)
                return null;
    ...
    public async Task<List<OrderInRedis>> GetUnFinishedOrdersInRedisAsync()
    {
        try
        {
            ...
            string[] orderCodesUnDone = _ecsRedisClient.GetHashFields(WcsConsts.UnFinishedDispatchOrderCodeChannel);
            foreach (string code in orderCodesUnDone)
            {
                if (string.IsNullOrEmpty(code))
                    continue;

                string strOrder = await _ecsRedisClient.GetHashValueAsync(WcsConsts.DispatchOrderChannel, code).ConfigureAwait(false);
    ...
    public async Task UpdateTaskInfoOfOrderInRedisAsync(string orderCode)
    {
        try
        {
            ...
            await _ecsRedisClient.SetHashValueAsync(WcsConsts.DispatchOrderChannel, orderCode, strDto).ConfigureAwait(false);
    ...
    public async Task UpdateOrderStateOfOrderInRedisAsync(string orderCode, string newOrderState)
    {
        try
        {
            ...
            if (newOrderState != EnumDispatchOrderState.Created.ToString() &&
                ...
                await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel, new string[] { orderCode }).ConfigureAwait(false);
            else //订单未完成
                await _ecsRedisClient.SetHashValueAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel, orderCode, string.Empty).ConfigureAwait(false);
    ...
    public async Task UpdateJobExecInfoOfOrderInRedisAsync(string orderCode, int jobId, string execInfo)
    {
        try
        {
            string orderInfo = await _ecsRedisClient.GetHashValueAsync(WcsConsts.DispatchOrderChannel, orderCode).ConfigureAwait(false);
            if (orderInfo == null)
                throw new Exception($"Redis数据库内没有订单号为{orderCode}的订单信息");
            ...
            await _ecsRedisClient.SetHashValueAsync(WcsConsts.DispatchOrderChannel, orderCode, orderInfo).ConfigureAwait(false);
    ...
    public async Task RemoveOrderInRedisAsync(string orderCode)
    {
        ...
        {
            await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel, new string[] { orderCode }).ConfigureAwait(false);
            await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.DispatchOrderChannel, new string[] { orderCode }).ConfigureAwait(false);
    ...
    public async Task RemoveOrdersInRedisAsync(List<string> orderCodes)
    {
        try
        {
            await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel, orderCodes.ToArray()).ConfigureAwait(false);
            await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.DispatchOrderChannel, orderCodes.ToArray()).ConfigureAwait(false);
    ...
    public async Task RemoveAllOrdersInRedisAsync()
    {
        try
        {
            await _ecsRedisClient.RemoveKeyAsync(WcsConsts.UnFinishedDispatchOrderCodeChannel).ConfigureAwait(false);
            await _ecsRedisClient.RemoveKeyAsync(WcsConsts.DispatchOrderChannel).ConfigureAwait(false);
    ...
    public async Task SetChkOrderRsltInRedisAsync(DispatchChkOrderRslt rslt)
    {
        try
        {
            ...
            await _ecsRedisClient.SetHashValueAsync(
                WcsConsts.DispatchChkOdResultChannel,
    ...
    public async Task RemoveOrderRsltInRedisAsync(string chkOdCode)
    {
        try
        {
            await _ecsRedisClient.RemoveHashFieldsAsync(WcsConsts.DispatchChkOdResultChannel, new string[] { chkOdCode }).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            _logger.Error(ex.Message);
    ...
    public async Task<List<DispatchChkOrderRslt>> GetChkResltsByOrderCodeInRedisAsync(string orderCode)
    {
        try
        {
            KeyValuePair<string, string>[] pairs = await _ecsRedisClient.GetAllHashFieldValuePairsAsync(WcsConsts.DispatchChkOdResultChannel).ConfigureAwait(false);
            if (pairs.Length == 0)
