...
namespace Wcs.Jobs.JobCmds
{
    public class LMOutPickCmd : IJobCmd, ITransientDependency
    {
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            ...
            //获取命令值
            var cmd = _nodeManager.GetNodeCmdAsync("13", WcsConsts.NodeType_LMOutPick).GetAwaiter().GetResult();
            if (cmd == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询龙门的出库取货命令信息失败" };
            ushort cmdNo = (ushort)cmd.NodeCmdValue;

            //获取按电气定义的目标库位排、列、层，以及取档口
            bool ret = _jobCmdHelper.GetPlcCellXYZOfStockOutTask(
                Owner.MyJob.TaskId,
                out ushort usRow, out ushort usLayer, out ushort usSectNo, out ushort usColInSect, out int iSpecsValue,
                out ushort doorCode,
        ...
        public OpResultInDispatchSvc SendCmdValue()
        {
            try
            {
                ...
                if (mCmdValue == null || mCmdValue.Length == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"龙门出库取货\"命令值空" };
                ...
                string log = Owner.GenerateLog($"向{plcName}.{tagName}发送龙门出库取货指令{cmdForLog}成功");
                _logger.Info(log);
                return new OpResultInDispatchSvc() { IsOK = true, Message = null };
            }
            catch (Exception ex)
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            try
            {
                ...
                var cmd = _nodeManager.GetNodeCmdAsync("13", WcsConsts.NodeType_LMOutPick).GetAwaiter().GetResult();
                if (cmd == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询龙门出库取货命令信息失败" };
                ushort nodeCmdNo = (ushort)cmd.NodeCmdValue;

                if (cmdNo != nodeCmdNo)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈当前执行的指令值为{cmdNo}，而非\"龙门出库取货\"指令{nodeCmdNo}" };

                if (taskState == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈尚未执行\"龙门出库取货\"指令" };
                else if (taskState == 1)
                {
                    if (taskNo != job.Id)
                        return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈正在执行JobID为{taskNo}的\"龙门出库取货\"指令，而不是ID为{job.Id}的当前Job的\"龙门出库取货\"指令" };
                    else
                ...
                {
                    if (taskNo != job.Id)
                        return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈JobID为{taskNo}的\"龙门出库取货\"指令已执行完成，而不是当前执行的JobID:{job.Id}" };
                    else
                    {
