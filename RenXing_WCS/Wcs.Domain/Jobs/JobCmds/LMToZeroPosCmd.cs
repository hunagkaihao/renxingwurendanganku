...
namespace Wcs.Jobs.JobCmds
{
    public class LMToZeroPosCmd : IJobCmd, ITransientDependency
    {
        ...
        public LMToZeroPosCmd(
            ILogger<LMToZeroPosCmd> logger,
            PlcHelper plcHelper,
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            ...
            //获取命令值
            var cmd = _nodeManager.GetNodeCmdAsync("13", WcsConsts.NodeType_LMToZeroPos).GetAwaiter().GetResult();
            if (cmd == null)
                return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询龙门回原点命令信息失败" };
        ...
        public OpResultInDispatchSvc SendCmdValue()
        {
            try
            {
                ...
                if (mCmdValue == null || mCmdValue.Length == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"龙门回原点\"命令命令值为空" };
                ...
                _logger.Info(Owner.GenerateLog($"向{plcName}.{cmdTagName}发送龙门回原点指令{cmdForLog}成功"));
                return new OpResultInDispatchSvc() { IsOK = true, Message = null };
            }
            catch (Exception ex)
            {
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            ...
            {
                if (Owner == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"龙门回原点\"命令没有指定所属的JobWorker信息" };

                if (Owner.MyJob == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = "\"龙门回原点\"命令所属Job为空" };
                ...
                var nodeCmd = _nodeManager.GetNodeCmdAsync("13", WcsConsts.NodeType_LMToZeroPos).GetAwaiter().GetResult();
                if (nodeCmd == null)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询龙门回原点命令信息失败" };
                ushort nodeCmdNo = (ushort)nodeCmd.NodeCmdValue;

                if (cmdNo != nodeCmdNo)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈当前执行的指令值为{cmdNo}，而非\"龙门回原点\"指令{nodeCmdNo}" };

                if (jobState == 0)
                    return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈尚未执行\"龙门回原点\"指令" };
                else if (jobState == 1)
                {
                    if (jobNo != job.Id)
                        return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈正在执行JobID为{jobNo}的\"龙门回原点\"指令，而不是ID为{job.Id}的当前Job的\"龙门回原点\"指令" };
                    else
                        return new OpResultInDispatchSvc() { IsOK = false, Message = "设备正在执行\"龙门回原点\"指令" };
                }
                else if (jobState == 2)
                {
                    if (jobNo != job.Id)
                        return new OpResultInDispatchSvc() { IsOK = false, Message = $"设备反馈JobID为{jobNo}的\"龙门回原点\"指令已执行完成，而不是当前执行的JobID:{job.Id}" };
                    else
                    {
