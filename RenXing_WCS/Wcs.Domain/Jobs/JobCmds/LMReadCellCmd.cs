...
namespace Wcs.Jobs.JobCmds
{
    public class LMReadCellCmd : IJobCmd, ITransientDependency
    {
        ...
        public OpResultInDispatchSvc GenerateCmdValue()
        {
            ...
            //获取对应的Cmd定义，并获取命令值
            var cmd = _nodeManager.GetNodeCmdAsync("13", WcsConsts.NodeType_ReadCell).GetAwaiter().GetResult();
            if (cmd == null)
        ...
        public OpResultInDispatchSvc IsCmdFinished()
        {
            try
            {
                ...
                // var nodeCmd = _nodeManager.GetNodeCmdAsync("13", WcsConsts.NodeType_ReadCell).GetAwaiter().GetResult();
                // if(nodeCmd == null)
                //     return new OpResultInDispatchSvc() { IsOK = false, Message = $"查询龙门读库位命令信息失败" };
