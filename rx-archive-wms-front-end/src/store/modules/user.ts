import type { UserInfo , WareInfo } from '/#/store';
...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getCell():string{
        return this.cell || getAuthCache<string>(WARE_KEY);
    },
    getWareIdByName():number{
      return (warename) => (this.ware.find((user) => user.warename === warename))?.wareid 
    },
    getWare():WareInfo[]{
      return this.ware || getAuthCache<WareInfo[]>(WARES_KEY);
    },
  ...
  actions: {
    ...
    setCell(cell: string){
      this.cell = cell;
      setAuthCache(WARE_KEY, cell);
    },
    setWare(ware: WareInfo[]){
      this.ware = ware;
      setAuthCache(WARES_KEY, ware);
    },
    resetState() {
      this.userInfo = null;
      this.roleList = [];
