...
namespace WarehouseManagement.CheckHiss
{
    public class CheckDetailHisManager : CheckDetailHisDomainService
    {
        ...
        /// TaskId 对应盘点时创建的 StockTask.Id，可作为轮询重复回传时的幂等依据。
        /// </summary>
        public async Task<bool> ExistsByTaskIdAsync(int taskId)
        {
            List<CheckDetailHis> histories = await _repository.GetListAsync(x => x.TaskId == taskId);
