...
ajax.interceptors.request.use(function (config) {
    ...
    // 新版 ABP 使用 __tenant 与 Accept-Language 解析租户、语言。
    const tenantId = window.abp.multiTenancy.getTenantIdCookie();
    if (tenantId) config.headers.common['__tenant'] = tenantId;
    const culture = window.abp.utils.getCookieValue('Abp.Localization.CultureName');
    if (culture) config.headers.common['Accept-Language'] = culture;
    return config;
  }, function (error) {
