...
import UserFingerVein from  '../entities/userFingerVein'

interface UserState extends ListState<User>{
    editUser:User,
    roles:Role[],
    userFingerVeins:UserFingerVein[],
    faceUser:User
}
...
class UserModule extends ListModule<UserState,any,User>{
    state={
        ...
        userFingerVeins:new Array<UserFingerVein>(),
        faceUser:new User()
    }
    actions={
        ...
        async getUrl(context:ActionContext<UserState,any>,payload:any){
            let reponse=await Ajax.get('/api/services/app/User/Get?Id='+payload.id);
            context.state.faceUser= reponse.data.result as User;
        },
        ...
        async imageUpload(content: ActionContext<UserState, any>, payload: any) {
            let rep = await Ajax.post("/api/services/app/User/ImageUpload", payload.data,{headers:{'Content-Type':'multipart/form-data'}});
        },
        async getVeins(context:ActionContext<UserState,any>,payload:any){
            let reponse=await Ajax.get('/api/services/app/User/GetVeinsByUserId?userId='+payload.userId);
            context.state.userFingerVeins=reponse.data.result as UserFingerVein[];
        },
        async deletephoto(context:ActionContext<UserState,any>,payload:any){
            await Ajax.delete('/api/services/app/User/DeleteUserPhoto?userId='+payload.userId);
        },
        async facePhotoUpload(context:ActionContext<UserState,any>,payload:any){
            await Ajax.post('/api/services/app/User/ImageUploadByStr',payload.data);
        },
        async getAllByClient(context:ActionContext<UserState,any>,payload:any){
            context.state.loading=true;
            let reponse=await Ajax.get('/api/services/app/User/GetAllByClient',{params:payload.data});
            context.state.loading=false;
            let page=reponse.data.result as PageResult<User>;
            context.state.totalCount=page.totalCount;
            context.state.list=page.items;
        },
        async deletevein(context:ActionContext<UserState,any>,payload:any){
            await Ajax.delete('/api/services/app/User/DeleteVeinDataById?id='+payload.id);
        },
    };
    mutations={
