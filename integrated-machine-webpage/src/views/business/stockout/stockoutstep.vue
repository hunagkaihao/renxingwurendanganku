    ...
    <div>
        <Card dis-hover  style="height:720px">
            <div class="page-body">
                ...
                <div v-show="current>=1" >
                        <Row>
                            ...
                            <ButtonGroup>
                                ...
                                <Button  v-show="current==1" icon="ios-remove" type="primary" size="large" @click="taskCancel"></Button>
                                <Button  v-show="current>1" icon="ios-refresh" type="primary" size="large" @click="getpage"></Button>
                                <Button v-show="current==5" icon="ios-apps-outline" type="primary" size="large" @click="taskConfirm" class="toolbar-btn">{{L('确认完成')}}</Button>
                                <!-- <Button  v-show="current==2" icon="ios-expand" type="primary" size="large" @click="taskConfirmBack" class="toolbar-btn">{{L('完成[回库]')}}</Button> -->
                                <Button  v-show="current>1" icon="ios-checkbox" type="primary" size="large" @click="taskComplete" ></Button>
                ...
                <div class="margin-top-20">
                   <Row :gutter="24">
                    <Col v-show="current>1&&current<6" span="8">
                        <h4>出库柜门</h4>
                         <div class="margin-top-10">
                    ...
                    <div id="achiveBox3" class="achiveBox" :style="{'left': (2*boxLocationL+100)+'px'}" >
                        <svg  width="60px" height="160px" viewBox="0 0 120 320" xmlns="http://www.w3.org/2000/svg">
                        <!-- Created with Method Draw - http://github.com/duopixel/Method-Draw/ -->
                        <g>
                        <title>Layer 1</title>
...
<script lang="ts">
    ...
    export default class Stocktasks extends AbpBase{
        ...
        continuousFlag:boolean=false;//连续入库标识
            ...
            content:'已完成',icon:'md-finger-print'},{ id:'2',title:'任务下达',
            content:'',icon:'ios-list-box'},{ id:'3',title:'密集柜开启',
            content:'',icon:'ios-albums'},{ id:'4',title:'机械手抓取',
            content:'',icon:'md-download'},{ id:'5',title:'档案取出',
            content:'',icon:'ios-person'},{ id:'6',title:'确认任务',
            content:'',icon:'md-checkbox'},{ id:'7',title:'任务结束',
       ...
       getRobotC(k:number)
        ...
        goForward()
        {
            ...
            if(this.current>0)
            {
                ...
                //console.log("当前页:"+this.currentpg+"当前任务数"+this.$store.state.stocktask.list.length);
                 //this.pageChange(3);
        ...
        get stocktask(){
            ...
            {
                // if(this.$store.state.stocktask.list.filter(x=>x.manageStatus!='WaitingExecute').length>0)
                // {
                //     return this.$store.state.stocktask.list.filter(x=>x.manageStatus!='WaitingExecute')[0];
                // }
                // else
                // {
         ...
         get deviceStatusDto()
      {
                ...
                get cellZ()
        {
            ...
            if(Object.keys(this.stocktask).length>0)
            {
                ...
                {
                 cell=this.stocktask.startCellName.split('-');
                }
            }
        ...
        getStep(status:string)
        {
                ...
                if(status=="WaitingExecute")
                {
                    ...
                    this.stepdata[1].content="等待任务下达";
                }
                ...
                if(status=="StationOpen")
                {
                    ...
                    this.stepdata[2].content="已完成";
                    this.stepdata[3].content="已完成";
                    this.stepdata[4].content="柜门已打开，请取档";
                }
        ...
        async  selectedCell()
        {
            ...
            console.log(this.stocktask.endCellName);
            if(this.stocktask.endCellName!="")
            {
                let stationDiv = document.getElementById(this.stocktask.endCellName);
                    console.log(stationDiv);
                    stationDiv.classList.add('Selected');
                    //this.box2=false;
            }
            if(this.stocktask.startCellName!=""&&this.current>=2)
            {

                let cell=this.stocktask.startCellName.split('-');
                console.log(cell);
                this.cellZYDto.cell_z=parseInt(cell[0]);
                this.cellZYDto.cell_y=parseInt(cell[2]);
                let aa=parseInt(cell[1])/13;
                ...
                //moveBox1.offsetLeft
                    ...
                    style.insertRule("@keyframes achive-move{0%{ transform: translate(0px,0px); }20%{ transform: translate(0px,"+yparam+"px); }50%{ transform: translate("+xparam+"px,"+(yparam)+"px);}80%{ transform: translate("+xparam+"px,"+yy+"px);}100%{ transform: translate("+xparam1+"px,"+yy+"px);}}",1);//写入样式
             ...
             console.log("当前排："+this.stocktask.startCellName);
            console.log("当前排："+parseInt(this.stocktask.startCellName.substring(1,2)));
        }
        //动画执行标识
        myStartFunction()
        {
      ...
      {  
        ...
        async taskConfirm(){  
            if(this.stocktask.id>0)
            {
                var gids=new Array<number>();
                 var plist=  new pickDto(this.stocktask.id,gids);
                        await this.$store.dispatch({
        ...
        async created(){
            ...
            setInterval(this.timedJob, 5000);
