...
<script lang="ts">
   ...
   import Archivebox from '@/store/entities/archivebox';
    @Component
    export default class BindingArchiveBox extends AbpBase{
            ...
            (this.$refs.archiveBoxForm as any).validate(async (valid:boolean)=>{
                if(valid){
                    await this.$store.dispatch({
                        type:'archivebox/binding',
                        data:   {
                            "storageMainId": this.archivebox.id,
                            "archiveBoxRfid": this.archivebox.archiveBoxRfid
                        }
                    });
        ...
        visibleChange(value:boolean){
            if(!value){
                ...
                this.archivebox=Util.extend(true,{},this.$store.state.archivebox.bindingArchiveBox);
            }
        }
        archiveBoxRule={
